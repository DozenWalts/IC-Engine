...
struct NanoJPGLoader{            // Jpeg file parameter
  public:
    //FILE    *file;  // file
    BUFFER *B;
                    ...
                    for(H = 0; H < component_info[plane].h; H++) {
                        ...
                        for(y = 0; y < 8; y++)
                            for(x = 0; x < 8; x++) {
								...
								// BUG: corrupted jped caused scalev[plane] == 10000000
...
bool Load(BUFFER *IB) {
	if (IB->size == U32_MAX) return false;
	B = IB;
	width = height = 0;
	data = NULL;
	data_precision = num_components = restart_interval = 0;
	//LOG::msg("sizeof(component_info)=%i sizeof(component_info)=%i", sizeof(component_info), sizeof(component_info));// 72 ok
	memset(component_info, 0, sizeof(component_info));// *3);//PVS really owerflow??
	...
	// memset(&jpeg_file_s,0,sizeof(jpeg_file_s));
	 //file = fopen(name,"rb");//LOG::MSG("load_jpeg CWfopen %s",name);//fopen(name,"rb");
	 //if(!file){return 0;};
	if (!jpeg_readmarkers()) return false;
